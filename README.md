...
## What It Does
When compaction risk is high, the skill tells Codex to warn before starting risky work and provide a concrete handoff that includes files to read, current state, commands run, validation status, constraints, and next steps. The ASCII warning banner is mandatory once risk has been identified, including before docs-only edits; prose alone is not enough. If a compaction or compaction-risk warning banner appears in an interim update, the final answer must repeat the same fenced ASCII warning so it remains visible after progress details collapse. The warning is based on practical risk signals, not one exact percentage. If a context percentage or token meter is visible, 75 percent is the conservative warning point because it leaves room to capture durable state before hard compaction; at or above 80 percent, warn before any non-trivial implementation, validation, generated-output, media, or docs phase.
